...
(async()=>{
 ...
 const accountHeight=await box.evaluate(el=>el.getBoundingClientRect().height);
 await box.getByRole('button',{name:'Пополнить',exact:true}).first().click();
 let transfer=page.getByRole('dialog',{name:'Пополнить'});await transfer.waitFor();
 assert.equal(await box.evaluate(el=>el.getBoundingClientRect().height),accountHeight,'profile card must not expand when opening a transfer');
 assert.equal(await box.locator('.ba-transfer-input').count(),0,'transfer form stays outside the profile');
 const picker=transfer.getByRole('button',{name:'Откуда пополнить'});
 assert.equal(await transfer.getByRole('option').count(),0,'source choices stay collapsed until opened');
 await picker.click();assert.equal(await transfer.getByRole('option').count(),2);
 await transfer.getByRole('option',{name:/Из MetaMask/}).click();
 assert.equal(await transfer.getByRole('option').count(),0);
 assert.match(await transfer.innerText(),/На MetaMask нет pUSD в Polygon\. Выберите счёт Polymarket или пополните MetaMask именно pUSD/);
 await picker.click();await transfer.getByRole('option',{name:/Со счёта Polymarket/}).click();
 await transfer.getByLabel('Сумма pUSD',{exact:true}).fill('75');await transfer.getByRole('button',{name:'Проверить перевод',exact:true}).click();
 await transfer.getByText('доступно 72.554481 pUSD, нужно 75 pUSD',{exact:false}).waitFor();
 ...
 await picker.click();await transfer.getByRole('option',{name:/Из MetaMask/}).click();
 await transfer.getByLabel('Сумма pUSD',{exact:true}).fill('1');await transfer.getByRole('button',{name:'Проверить перевод',exact:true}).click();
 await transfer.getByText('доступно 0 pUSD, нужно 1 pUSD',{exact:false}).waitFor();
 assert.equal(mutations(),0);assert.deepEqual(await page.evaluate(()=>window.__walletMethods),[]);
 ...
 ({page,box}=await open());
 await box.getByRole('button',{name:'Пополнить',exact:true}).first().click();
 transfer=page.getByRole('dialog',{name:'Пополнить'});await transfer.waitFor();
 await transfer.getByRole('button',{name:'Откуда пополнить'}).click();await transfer.getByRole('option',{name:/Из MetaMask/}).click();
 await transfer.getByLabel('Сумма pUSD',{exact:true}).fill('10');await transfer.getByRole('button',{name:'Проверить перевод',exact:true}).click();
 await transfer.getByText('Перевод готов — подтвердите в MetaMask',{exact:false}).waitFor();
